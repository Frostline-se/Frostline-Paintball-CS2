...
public sealed class FrostlinePaintballPlugin : BasePlugin
{
    ...
    private void SpawnDecal(Vector position, QAngle angles, float size, string material)
    {
        ...
        {
            LogSpawnErrorOnce(exception, Core.Localizer["frostline_paintball.errors.decal_create_failed"]);
            return;
        }
        ...
        catch (Exception exception)
        {
            ...
            LogSpawnErrorOnce(exception, Core.Localizer["frostline_paintball.errors.decal_spawn_failed"]);
        }
    }
