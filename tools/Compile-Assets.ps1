...
function Initialize-GeneratedDirectory {
    param(
...
Initialize-GeneratedDirectory -Path $contentRoot -AllowedRoot (Join-Path $Cs2Root 'content\csgo_addons')
Initialize-GeneratedDirectory -Path $gameRoot -AllowedRoot (Join-Path $Cs2Root 'game\csgo_addons')
...
Initialize-GeneratedDirectory -Path $releaseAssets -AllowedRoot (Join-Path $projectRoot $config.ReleaseDir)
